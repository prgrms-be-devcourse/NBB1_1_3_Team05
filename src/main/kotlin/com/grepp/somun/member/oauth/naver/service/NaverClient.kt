...
@Service
class NaverClient(
    @Value("\${naver.client_id}") private val clientId: String,
    @Value("\${naver.redirect_uri}") private val redirectURI: String,
    @Value("\${naver.client_secret}") private val clientSecret: String,
    ...
    override fun getAccessToken(code: String, state: String): String {
        ...
        return jsonObject["access_token"].asString
    }
    ...
    override fun getMemberInfo(accessToken: String): SocialMemberCheckDto {
        ...
        val memberInfoRequest = HttpEntity<MultiValueMap<String, String>>(httpHeaders)
        val response: ResponseEntity<String> = restTemplate.exchange(
            reqUrl,
            HttpMethod.POST,
            memberInfoRequest,
        ...
        // JSON 파싱
        val jsonObject: JsonObject = JsonParser.parseString(response.body!!).asJsonObject
